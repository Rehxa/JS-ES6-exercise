  ...
  for (const cart of SHOPPING_CART) {
    for (const product of PRODUCTS) {
      if (cart.id === product.id) {
        result += cart.quantity * product.price;
      }
    }
  }
  return result;
...
function addProductToCart(productId) {
  // Write your code here
  let checkId = SHOPPING_CART.find((inCart) => inCart.id == productId);
  if (checkId) {
    checkId.quantity++;
  } else {
...
function removeProductFromCart(productId) {
  // Write your code here
  let checkId = SHOPPING_CART.findIndex((inCart) => inCart.id == productId);
  if (checkId !== -1) {
    if (SHOPPING_CART[checkId].quantity > 1) {
      SHOPPING_CART[checkId].quantity--;
    } else {
      SHOPPING_CART.splice(checkId, 1);
    }
  }
}
